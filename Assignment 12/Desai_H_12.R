...
dbGetQuery(conn=dbBird, "SELECT COUNT(*) AS Total_Birdstrike_AmericanAirlines FROM BIRDSTRIKE
           JOIN FLIGHT ON BIRDSTRIKE.Flight_ID = FLIGHT.Flight_ID
...
dbGetQuery(conn=dbBird, " SELECT Airline,COUNT(*) AS Number_of_Strikes FROM BIRDSTRIKE
           JOIN FLIGHT ON BIRDSTRIKE.Flight_ID = FLIGHT.Flight_ID
           GROUP BY (Airline) 
...
dbGetQuery(conn=dbBird , " SELECT Airline AS Most_Birdstrikes_By_Airline FROM 
            (SELECT Airline,COUNT(*) AS Number_of_Strikes FROM BIRDSTRIKE
            JOIN FLIGHT ON BIRDSTRIKE.Flight_ID = FLIGHT.Flight_ID
            WHERE Airline NOT IN ('UNKNOWN','MILITARY')
            GROUP BY (Airline)
            ORDER BY (Number_of_Strikes) DESC) LIMIT 1")
#Query 4 
#Which bird strikes occurred for Helicopters? List all the bird strike incidents with date.

dbGetQuery(conn=dbBird ," SELECT Aircraft_Type,Reported_Date FROM BIRDSTRIKE AS A
           JOIN AIRCRAFT AS B ON A.Aircraft_ID = B.Aircraft_ID 
...
dbGetQuery(conn=dbBird , " SELECT Airline AS List_of_Airlines FROM 
            (SELECT Airline,COUNT(*) AS Number_of_Strikes FROM BIRDSTRIKE
           JOIN FLIGHT ON BIRDSTRIKE.Flight_ID = FLIGHT.Flight_ID
           WHERE Airline NOT IN ('UNKNOWN','MILITARY')
           GROUP BY (Airline)
           ORDER BY (Number_of_Strikes) DESC) WHERE Number_of_Strikes > 10")
